...
from functions.denoising import generalized_steps,generalized_steps_ret_h,generalized_steps_ret_clip,mod_generalized_steps_ret_h
...
class Diffusion(object):
    ...
    def sample(self,mode):
        ...
        model.eval()
        self.sample_fid(model,mode,phase_3=False) ## Toggle
            ...
            for _ in tqdm.tqdm(range(n_rounds), desc="Generating"):
                ...
                if mode == 0: 
                    ...
                    if not self.args.collect: 
                        ...
                        if phase_3:
                            x,proj_clip = self.sample_image_debias(x, model,ret_h = False,ret_clip=True)
                            x = inverse_data_transform(config, x)
                            ...
                            x,h_un_mod,h_mod,kl_per_timestep = self.sample_image_debias(x, model,ret_h = False,ret_clip=False)
                            kl_track = sorted(kl_per_timestep)  # Sort by timestep
                            timesteps, kl_values = zip(*kl_track)
                            plt.figure(figsize=(10, 6))
                            plt.plot(timesteps, kl_values, marker='o')
                            plt.xlabel("Timestep")
                            ...
                            for i in range(n):
                                tvu.save_image(x[i], os.path.join(self.args.output, "images" ,f"{img_id}.png"))
                                # h_path_unmod = os.path.join(self.args.output, "h_unmod" ,f"{img_id}.pt")
                                # h_path_mod = os.path.join(self.args.output, "h_mod" ,f"{img_id}.pt")
                                # h_i_unmod = [h[i].clone() for h in h_un_mod]
                                # torch.save(h_i_unmod, h_path_unmod)
                                # h_i_mod = [h[i].clone() for h in h_mod]
                                # torch.save(h_i_mod, h_path_mod)
                                img_id += 1
    ...
    def sample_image_debias(self,x, model, ret_h=False, ret_clip=False):
        try: skip = self.args.skip
        except Exception: skip = 1
        skip = self.num_timesteps // self.args.timesteps
        seq = range(0, self.num_timesteps, skip)
        if not ret_h and ret_clip: 
            ...
            xs,h_unmod,h_mod,kl_per_timestep,_ = mod_generalized_steps_ret_h(x, seq, model, self.betas, eta=self.args.eta, model_path="checkpoints/htoclip_joint_final.pt", centroids_path="cluster_centroids.npy", edit_range=(10,40), temperature=0.07, gamma=10.0)
